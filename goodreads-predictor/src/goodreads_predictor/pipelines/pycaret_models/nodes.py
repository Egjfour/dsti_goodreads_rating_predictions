...
def train_and_predict_models(train_data: pd.DataFrame, test_data: pd.DataFrame, features: List[str], target: str, config: Dict[str, Any]) -> pd.DataFrame:
    ...
    # bayesian_model = train_bayesian_ridge(X_train, y_train, config['bayesian_params'], cv_eval)
    ridge_model = train_ridge(X_train, y_train, config['ridge_params'], cv_eval)
    bayesian_model = ridge_model
